...
export class MockXHRBackend implements HttpBackend {
  ...
  _deleteMediaItem(id: any) {
    const mediaItem = this.mediaItems.find((i: { id: any }) => i.id === id);
  ...
  _getNewId() {
    if (this.mediaItems.length > 0) {
      return (
        Math.max.apply(
          Math,
          this.mediaItems.map((mediaItem: { id: any }) => mediaItem.id)
        ) + 1
      );
    } else {
      return 1;
